...
results_30 <- data.frame("month"=0, "lambda"=0,"less_turnover"=0)
...
results_w_30 <- matrix(0,lenmonth,ncol(data1)-1)
...
for (month in 1:lenmonth) {
  ...
  w_idx <- which(colSums(round(ww[,-ncol(ww)],2)!=0)/p >= 0.3 & 0.4 >= colSums(round(ww[,-ncol(ww)],2)!=0)/p)
  if (length(w_idx) >= 2 & month >=2) {
    j <- 1
    comp <- rep(0,length(w_idx))
    for(i in w_idx ) {
      comp[j] <- sum(abs(results_w_30[month-1,]-round(ww[,i],2))) # turnover 계산
      j <- j+1
    }
  ...
  results_30 <- rbind(results_30, c(month, w_idx, nreturn))
  results_w_30[month,] <- www
