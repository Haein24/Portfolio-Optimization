...
cl = makeCluster(n_core-2) # 사용가능한 코어중 하나를 빼놓고 할당
cl
...
lenmonth <- 2 # 12*10 # 몇 년치 데이터를 다룰 것인지 : 10년치
...
for (month in 2:lenmonth) {
  ...
  ww <- foreach(lambda=iter(lambda, by='column'), .combine=cbind,.packages = c("quadprog","expm","glmnet")) %dopar% { #, .export=c("proxSortedL1", "proxZ")) %dopar% { #, .combine=cbind) {
    ...
    while (TRUE) {
      if ( month != 1) {
        ...
        X <- sqrtm(eq2)
        Y <- -(1/2)*(solve(X)%*%t(eq1 + 2*t(w_tilde)%*%eq2))
        
        w_ <- glmnet(X, Y, alpha=1, lambda=0.1)$beta[1:p]
        w <- w_+w_tilde
      } else {
  ...
  return_lasso <- N_daily_return[-ncol(data1)]%*%round(ww[,31],1)
