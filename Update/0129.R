...
title <- paste("data/",data_set[6],".xlsx", sep="")
print(title)
...
for ( z in 1:ncol(lambda) ){
  ...
  e <- rep(1,k)
  II <- diag(k)
  theta <- k#/10 # 상대위험회피정도 : theta > 0 (여기서는 임의의 값으로 두었음,,)
  eta <- 0.03 # 1/ncol(base) # eta > 0 (여기서는 임의의 값으로 두었음,,)
  j <- 0
  tau <- 10^(-4)
  lambda_1 <- lambda[,z]
...
plot(ww[1,], ylim=c(min(ww),max(ww)), type='l', col=1, lwd=1, main=paste("data = ", title, ", theta = ", theta, ", eta = ", round(eta,3)))
