...
turnover <- function(results_w) {
  turnover_prep <- rep(0, nrow(results_w)-2)
  for (i in 1:(nrow(results_w)-2)) {
    turnover_prep[i] <- sum(abs(results_w[i+2,]-results_w[i+1,]))
  }
...
fb <- function(w, ll) { 
  ...
  t(w)%*%(eq2)%*%w + (eq1 + c(2)%*%t(w_tilde)%*%eq2)%*%w +ll*sum(abs(w))
}

# subgradient 구하는 함수
subfb = function(w, ll) {
  front =  c(1/2)*t(eq1) + t(eq2)%*%w_tilde + (eq2)%*%w
  subg = NULL
  for (i in 1:length(w)) {
    if ( w[i] > 0 ) {
      subg[i] = front[i]  + ll
...
lenmonth <- 12*3 #12*10 # 몇 년치 데이터를 다룰 것인지 : 10년치
...
results_30 <- results_w_17 <- results_w_las <- rep(0,ncol(data1)-1)
...
for (month in 1:lenmonth) {
  ...
  ww <- foreach(lambda=iter(lambda, by='column'), .combine=cbind,.packages = "quadprog") %dopar% {
    ...
    while (TRUE) {
      if ( month == 0) {
        ...
        bestbeta <- pbeta1 <- rep(0,p) # 초기값은 0으로 주기(아닌가)
        mat <- t(matrix(pbeta1)) # 시각화용
        ...
        while ( iter < 200 ) { 
          ...
          t <- 0.01/iter # step size
          nbeta <- pbeta1 - t/iter*subfb(pbeta1, p*lamb) # next beta  ## 문제 되던곳 찾음!!!
  ...
  results_30 <- rbind(results_30,round(ww[,w_idx],2))
  results_w_17 <- rbind(results_w_17,round(ww[,17],2))
  results_w_las <- rbind(results_w_las, round(ww[,31],2))
  ...
  #print(c("return of the month : ", nreturn,return_17,return_lasso, idx, ew)) # 샤프지수 프린트
...
# 최종 output dataframe 만들기-SP
dt_result_S <- data.frame()
...
for ( i in 3:7) {
  ...
  dt_result_S <- rbind(dt_result_S, c(port, risk, mere, shr))
}

results_30 <- results_30[-1,]
results_w_17 <- results_w_17[-1,]
results_w_las <- results_w_las[-1,]

dt_result_S <- cbind(dt_result_S,c(round(turnover(results_30),4), round(turnover(results_w_17),4), round(turnover(results_w_las),4),NA, NA))
dt_result_S <- cbind(dt_result_S, c(round(setdiff(results_30),4), round(setdiff(results_w_17),4), round(setdiff(results_w_las),4), NA, NA))
dt_result_S <- cbind(dt_result_S, c(round(mean(rowSums(results_30 !=0)),4), round(mean(rowSums(results_w_17 !=0)),4), round(mean(rowSums(results_w_las !=0)),4), NA, p))

colnames(dt_result_S) <- c("Portfolio","Risk", "Mean_Return", "Sharpe_ratio", "Turnover", "Set_Diff", "AP")
View(dt_result_S)
